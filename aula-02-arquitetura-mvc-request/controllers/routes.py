    ...
    def consoles():
        if request.method == 'POST':
            if request.form.get('nome') and request.form.get('fabricante') and request.form.get('ano') and request.form.get('preco'):
                consolelist.append({'Nome' : request.form.get('nome'), 'Fabricante' : request.form.get('fabricante'), 'Ano' : request.form.get('ano'), 'Preço' : request.form.get('preco')})
            return redirect(url_for('consoles.html'))
